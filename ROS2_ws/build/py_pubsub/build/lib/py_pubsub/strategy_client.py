...
def mm_to_pixels(mmx, mmy):
    ...
    py = tableheight*mmy/tablewidth_mm
    return px, py
    ...
    def strategy_state(self, state:States) -> States:
        if state == States.INIT:
            ...
            if self.cue_hitpoint[-1] >= 5500:
                hit_pin = HITSOFT_PIN
            elif 5500 > self.cue_hitpoint[-1] >= 2000:
                hit_pin = HITMID_PIN
            else:
    ...
    def strategy_state(self, state:States) -> States:
        if state == States.INIT:
            ...
            for i in range(0, self.Nballinroute, 2):
                ...
                l = len(yolo_second_res)
                objx = yolo_second_res[0:l/2]
                objy = yolo_second_res[l/2:l]
                ...
                for j in range(0,l/2):
                    ...
                    disy =objy[j] - tableheight/2 
                    temp_dis = math.sqrt((disx)**2+(disy)**2)
                    dis.append(temp_dis)
                midball_index = dis.index(min(dis))
                devx = disx[midball_index]
                devy = disy[midball_index]
                ...
                ball_position_pixels = []
                for j in range(0, len(real_ball_position), 2):
                    tempx, tempy = mm_to_pixels(real_ball_position[j], real_ball_position[j+1])
                    ball_position_pixels.append(tempx)
                    ball_position_pixels.append(tempy)
                    
            state = States.RECALCULATE

        elif state == States.RECALCULATE:
            self.cue_hitpoint = []
            self.yolo_req.update_position = ball_position_pixels
            ...
            if self.cue_hitpoint[-1] >= 5000:
                hit_pin = HITSOFT_PIN
            elif 5000 > self.cue_hitpoint[-1] >= 2000:
                hit_pin = HITMID_PIN
            else:
                hit_pin = HITHEAVY_PIN
