...
def check_obstacle(ballx, bally, vectorx, vectory, objx, objy):
    maskheight = tableheight/actualheight*39+2*r
    maskwidth = tablewidth/actualwidth*87+2*r
    vectorlengh = math.sqrt(abs(vectorx)**2+abs(vectory)**2)
    unit_vector = np.array([vectorx/vectorlengh, vectory/vectorlengh])
    normal_unit_vector = np.array([vectory/vectorlengh, vectorx/vectorlengh])
    ...
    # polygon
    polygon = mplPath.Path(np.array([[first_poly],[second_poly],[third_poly],[fourth_poly]]))
    obj = np.zeros(len(objx), 2)
    for i in range(0,len(objx)):
        obj[i][0] = objx[i]
        obj[i][1] = objy[i]
    obstacle_flag = polygon.contains_point(obj)
...
def main(cuex, cuey, objectballx, objectbally, n):
    ...
    return  ValidRoute, bestrouteindex 
